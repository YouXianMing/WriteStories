...
@interface FolderWifiViewController () <GCDWebUploaderDelegate>
...
@implementation FolderWifiViewController
...
- (void)viewDidLoad {
    ...
    // 文件存储位置
    NSString* documentsPath = FoldersManager.FolderBackups;
    ...
    // 限制文件上传类型
    self.webServer.allowedFileExtensions = @[@"folder"];
    
    // 设置网页标题
    self.webServer.title = @"美记";
    
    // 设置展示在网页上的文字(开场白)
    self.webServer.prologue = @"<p>欢迎使用美记WIFI管理平台</p><p>注意，上传的文件格式必须为[年-月-日 时-分-秒] 文件夹名字.folder，月、日、时、分、秒均为2位数（例如：[2018-05-05 09-47-30] 文件夹.folder），否则上传无法成功！</p>";
