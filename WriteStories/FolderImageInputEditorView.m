...
@interface FolderImageInputEditorView () <UIImagePickerControllerDelegate, UINavigationControllerDelegate, CustomViewControllerDelegate>
...
@implementation FolderImageInputEditorView
...
- (void)setInputEdior:(InputEditor *)inputEdior {
    ...
    NSString *imagePath          = [NSString stringWithFormat:@"%@/content/images/%@", FoldersManager.WorkShop, imageObject.imageName];
...
- (void)baseCustomViewController:(__kindof CustomViewController *)controller event:(id)event {
    ...
    EncodeImageObject *imageObject = [self.inputEdior valueWithObject:self.weakObject];
    NSString *imagePath            = [NSString stringWithFormat:@"%@/content/images/%@",
