...
#import "FolderWifiViewController.h"
...
@interface FolderImportViewController () <UITableViewDelegate, UITableViewDataSource, CustomViewControllerDelegate>
...
@implementation FolderImportViewController
...
- (void)viewDidLoad {
    ...
    if (self.currentFoldersCount >= 50) {
        
        [GCDQueue executeInMainQueue:^{
            
            [WSAlertView showAlertViewWithMessage:@"注意，每个大分类中最多只能有50个文件夹！"
...
- (void)reCreateData {
    ...
    // 获取数据源
    File *file = [FileManager scanRelativeFilePath:@"~/Documents/文件夹备份" maxTreeLevel:1];
    ...
    [file.subFiles enumerateObjectsUsingBlock:^(File * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (obj.isDirectory == NO &&
            ...
            [obj.filenameExtension isEqualToString:@"folder"] &&
            ...
            if (date) {
                ...
                obj.date         = date;
                obj.showName     = [obj.fileName substringWithRange:NSMakeRange(22, obj.fileName.length - 22 - 7)];
...
- (void)buttonsEvent:(UIButton *)button {
    ...
    if (button.tag == TitleMenuButton_Wifi) {
        ...
        NSLog(@"wifi");
        FolderWifiViewController *controller = [FolderWifiViewController new];
        ...
        NSMutableArray <BaseFolderItem *> *importItems = [NSMutableArray array];
            ...
            [self.files enumerateObjectsUsingBlock:^(File *file, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                if (file.selected) {
                    ...
                    if /* 有密码 */ ([SSZipArchive isFilePasswordProtectedAtPath:file.filePath]) {
                        ...
                        NSError *error = nil;
                        [SSZipArchive isPasswordValidForArchiveAtPath:file.filePath password:@"write.stories.you.xian.ming.folder" error:&error];
                                ...
                                if /* 文件版本检测成功 */ ([VersionManager checkVersionWithFolder:[NSString stringWithFormat:@"%@/content", folderPath]]) {
                                    ...
                                    [Table_Folder_List storeData:data
                                                      ...
                                                      folderName:folderName
                                                       sortIndex:[Table_Folder_List maxSortIndexWithType:self.type.integerValue] + 1];
                                    
                                    // 获取带有数据库信息的版本
                                    folderItem = [Table_Folder_List itemWithFolderName:folderName type:self.type.integerValue];
                                    
                                    // 添加数据
                                    [importItems addObject:folderItem];
...
- (void)updateTitleLabel {
    ...
    self.titleLabel.text = [NSString stringWithFormat:@"导入 (%ld/%ld)", (long)count, (long)(50 - self.currentFoldersCount)];
    
    if (count <= 0 || count > 50 - self.currentFoldersCount) {
...
- (void)baseCustomViewController:(__kindof CustomViewController *)controller event:(id)event {
    ...
    if ([controller isKindOfClass:FolderWifiViewController.class]) {
