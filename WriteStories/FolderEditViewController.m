...
@interface FolderEditViewController () <UICollectionViewDelegate, UICollectionViewDataSource, EditViewDelegate, CustomViewControllerDelegate, StyleManagerEditViewDelegate>
...
@implementation FolderEditViewController
...
- (void)viewDidLoad {
    ...
    self.collectionView                 = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.layout.itemSize.width + gap * 2.f, self.layout.itemSize.height + gap * 2)
    ...
    [self.folderItem registerToCollectionView:self.collectionView];
...
- (void)buttonsEvent:(TitleIconButton *)button {
    ...
    if (button.tag == TitleIconButtonIconType_InfoEdit) {
        ...
        FolderInputInfoEditViewController *controller = [FolderInputInfoEditViewController new];
        controller.title                              = @"信息编辑";
        controller.folderItem                         = self.folderItem.dbCopyObject;
        ...
        editView.editManager = self.folderItem.editManager;
        ...
        editView.styles                = [Table_Style_List styleObjectsByShowStyleType:[self.folderItem.class ShowStyleType]];
        editView.delegate              = self;
...
- (void)styleManagerEditView:(StyleManagerEditView *)editView selectedStyle:(WriteStoriesBaseItemObject *)style {
    ...
    [self.folderItem useStyleObject:style];
    
    // 获取cell
    BaseFolderItemCell *cell = (BaseFolderItemCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
...
- (void)editViewButtonEvent:(EditView *)editView {
    ...
    if ([Table_Style_List styleCountsByItemObject:self.folderItem] >= 10) {
    ...
    [StyleSaveView styleSaveViewShowWithText:@"自定义样式" delegate:nil confirmEvent:^(NSString *text) {
        ...
        [Table_Style_List storeStyleObject:self.folderItem styleName:text];
...
- (void)udpateDatas {
    ...
    [GCDQueue executeInGlobalQueue:^{
        ...
        // 获取二进制文件
        NSData *data = self.folderItem.encodeData;
        
        // 写数据
        NSString *path = [FoldersManager folderData_FolderListWithFolderName:self.folderItem.folder_name];
        [data writeToFile:path atomically:YES];
        
        // 更新数据库
        [Table_Folder_List updateData:data
