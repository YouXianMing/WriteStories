...
/**
 *  Transform relative file path to absolute file path.
 *
 *  @param relativeFilePath Relative file path, "~" means sandbox root, "-" means bundle file root.
 *
 ...
 *  @param absoluteFilePath The absolute file path.
 */
NS_INLINE void createDirectoryIfNotExistAtAbsoluteFilePath(NSString *absoluteFilePath) {
    
    if ([FileManager fileExistWithAbsoluteFilePath:absoluteFilePath] == NO) {
        
        [NSFileManager.defaultManager createDirectoryAtPath:absoluteFilePath withIntermediateDirectories:YES attributes:nil error:nil];
    }
}
